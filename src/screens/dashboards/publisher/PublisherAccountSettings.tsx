...
import { getPublisherProfile, updatePublisherAccount } from '../../../services/publisherServices';
...
const AccountSettings = () => {
    ...
    const initializePage = async () => {
        ...
        try {
            const profile = await getPublisherProfile(user.email);
            setUserDetails({
                fullName: { isEditing: false, value: profile.fullName },
                email: { isEditing: false, value: profile.email },
                gender: { isEditing: false, value: profile.gender },
                birthDate: { isEditing: false, value: profile.birthDate },
                phone: { isEditing: false, value: profile.phone },
                emergencyContact: { isEditing: false, value: profile.emergencyContact }
            });
        } catch (e) {
            console.error(e);
        }
                            ...
                            <div>
                                <h6 className='text-small text-black font-[700]'>Date of birth</h6>
                                {
                                    birthDate.isEditing ?
                                        ...
                                        />
                                        :
                                        <p className={`text-small mt-[3px] ${birthDate.value ? "text-black" : "text-silver"}`}>
                                            {
                                                birthDate.value ?
